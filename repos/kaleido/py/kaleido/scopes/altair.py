...
class AltairScope(BaseScope):
    """
    Scope for transforming Altair/Vega figures to static images
    """
    _all_formats = ("png", "svg", "pdf")
    _text_formats = ("svg")

    _scope_flags = ("vega")
    _scope_chromium_args = ("--no-sandbox",)

    def __init__(self,vegajs=None,**kwargs):
        self._vega = vegajs
        self.default_scale=1

        super(AltairScope,self).__init__(**kwargs)
    ...
    def transform(self, chart, format=None):
        """
        Convert an Altair/Vega figure into a static image

        :param chart: Altair chart or chart dictionary
    ...
    def vegajs(self):
        """
        URL or local file path vega.js bundle to use for image export.
